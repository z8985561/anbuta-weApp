...
Page({
  ...
  selectPicker: function(t) {
    ...
    if (this.data.goods.distributorstatus == 0) {
      e.confirm("需成为经销商才能进行购买", () => {
        wx.navigateTo({
          url: '/pages/dealer/apply/apply',
        })
      }, null)
      return;
    } else if (this.data.goods.distributorstatus == 3) {
      e.toast("正在审核中", "none");
      return
    } else if (this.data.goods.distributorstatus == 2) {
      e.confirm("申请被驳回，请修改申请资料", () => {
        wx.navigateTo({
          url: '/pages/dealer/apply/apply',
        })
      }, null)
    ...
    if (this.data.goods.levelbuy == 0) {
      e.toast("您还未成为经销商", "none");
      return;
    }
    let {
      buytype
    ...
    wx.navigateTo({
      url: `/pages/goods/picker-spec/picker-spec?id=${_id}&type=${buytype}`,
    })
    return;
    var a = this,
      o = t.currentTarget.dataset.tap,
